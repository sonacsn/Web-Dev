defmodule TaskTrackerWeb.TimeBlockController do
  ...
  def create(conn, %{"time_block" => time_block_params}) do
    with {:ok, %TimeBlock{} = time_block} <- Track.create_time_block(time_block_params) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"id" => id}) do
    time_block = Track.get_time_block!(id)
  ...
  def update(conn, %{"id" => id, "time_block" => time_block_params}) do
    ...
    with {:ok, %TimeBlock{} = time_block} <- Track.update_time_block(time_block, time_block_params) do
      render(conn, "show.json", time_block: time_block)
    end
  end
  ...
  def delete(conn, %{"id" => id}) do
    time_block = Track.get_time_block!(id)
    with {:ok, %TimeBlock{}} <- Track.delete_time_block(time_block) do
